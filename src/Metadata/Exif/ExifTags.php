<?php
	 ...
	 * @package Fawno\MetadataToolkit\Metadata\Exif
	 */
	class ExifTags {
		protected const Exif_TAGS = [
			'1#000' => [
		...
		public static function getTagName (string $tag) : string {
			return self::Exif_TAGS[$tag]['name'] ?? $tag;
		}

		public static function getTagFormat (string $tag) : string {
			return self::Exif_TAGS[$tag]['format'] ?? 'a*';
		}

		public static function getTagByName (string $name) : ?string {
			$tags = array_combine(array_column(self::Exif_TAGS, 'name'), array_keys(self::Exif_TAGS));
